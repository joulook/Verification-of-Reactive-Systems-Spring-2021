	...
	switch (propertySelector) {
			...
			switch(claim) {
							case 1:
																...
																break;
							case 0:
																							if (!_ref_mutex1) result.push_back(1);
																								if (!_ref_mutex2) result.push_back(1);
																								if (!_ref_mutex3) result.push_back(1);
																								result.push_back(0);
																break;
						}
		break;
			...
			switch(claim) {
							case 1:
																							if (!_ref_pw1&!_ref_pw2&!_ref_pw3) result.push_back(1);
																break;
							case 0:
																							if (!_ref_pw1&!_ref_pw2&!_ref_pw3) result.push_back(1);
																								result.push_back(0);
		...
		break;
			...
			switch(claim) {
							case 1:
																							if (!_ref_pw1) result.push_back(1);
																break;
							case 2:
																							if (!_ref_pw2) result.push_back(2);
																break;
							case 3:
																							if (!_ref_pw3) result.push_back(3);
																break;
							case 0:
																							if (!_ref_pw1) result.push_back(1);
																								if (!_ref_pw2) result.push_back(2);
...
void CoreRebecaModelChecker::doDFSModelChecking(string userSelectedPropertyName, string userSelectedPropertyType) {
	...
	depth = -1;	// in symmetry, depth+1 is used in store()
		...
		propertySelector = 0;
			else if (userSelectedPropertyName == "Safty") {
			propertySelector = 1;
			claim = 0;
		}
					else if (userSelectedPropertyName == "Deadlock") {
			propertySelector = 2;
			claim = 0;
		}
					else if (userSelectedPropertyName == "Liveness") {
			propertySelector = 3;
			claim = 0;
		}
				else {
